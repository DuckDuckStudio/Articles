...
参见 [No suitable installer found ...](#no-suitable-installer-found) 章节。  
...
如果 wingetbot / wingetvalidator-prod[bot] 有说明具体是哪个防病毒供应商的报告，请向对应供应商报告误报。如果没有，无解。  
...
可以试试使用 `winget download --ignore-security-hash` 命令下载安装程序后通过记事本打开它看看到底是什么问题。  
...
清单中的某些 URL 可能存在问题。wingetbot / wingetvalidator-prod[bot] 应该会定时重试验证管道，看看能否解决这个问题。  
...
### Validation-Domain
**验证管道无法从 `PackageUrl` 导向 `InstallerUrl`，需要说明你是从哪里获取到 `InstallerUrl` 的**。  
截图、`curl -IL` 等都是个好方法。  
...
### 验证管道日志在哪看？
#### 使用 wingetvalidator-prod[bot] GitHub App 进行的验证

直接在 PR 检查页中的 WinGetValidator-Prod 查看。

#### 之前使用 Azure DevOps 进行的验证
> [!CAUTION]
> 之前使用 ADO 进行验证的方式已弃用，自 2026 年 8 月 10 日起改用 wingetvalidator-prod[bot] GitHub App。  
> 更多信息，请参阅 [microsoft/winget-pkgs#263724](https://github.com/microsoft/winget-pkgs/issues/263724)。
...
##### 使用 wingetvalidator-prod[bot] GitHub App 进行的验证
> [!TIP]
> 如果验证是在 2026 年 8 月 10 日之前通过 ADO 进行的，请参阅下面的 [之前使用 Azure DevOps 进行的验证](#之前使用-azure-devops-进行的验证) 部分。

选择 **WinGetValidator-Prod / 10. Validation Completed** 这个检查，检查详情是一个 json 字典，json 字典中的 `Artifacts` 字典中的 `ArtifactDownloadUrl` 字段的值就是日志的下载链接。

###### 查看验证详情
**方法一：**从 PR 底部的检查查看详情
![从 PR 底部的检查查看详情](photos/GitHubApp/从PR底部的检查查看详情.png)

> “直接点进去”和“点 <svg data-component="Octicon" aria-hidden="true" focusable="false" class="octicon octicon-kebab-horizontal" viewBox="0 0 16 16" width="16" height="16" fill="currentColor" display="inline-block" overflow="visible" style="vertical-align: text-bottom;"><path d="M8 9a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3ZM1.5 9a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Zm13 0a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"></path></svg> 再点 View details 进去”是一样的。

**方法二：**从提交的检查查看详情

![从提交的检查查看详情](photos/GitHubApp/从提交的检查查看详情.png)

> 请注意这是对应提交的验证，而不是最新提交的验证。

**方法三：**从检查页面查看详情

![从提交的检查查看详情](photos/GitHubApp/从检查页面查看详情.png)

###### 从 json 中找到日志下载链接

![验证日志下载链接在json中的位置](photos/GitHubApp/验证日志下载链接在json中的位置.png)

> `Artifacts` 字典中的 `ArtifactDownloadUrl` 字段的值，在最底下。
...
请参考这条 GitHub 评论: [microsoft/winget-pkgs#355564 (评论)](https://github.com/microsoft/winget-pkgs/pull/355564#issuecomment-4196006181)  
以下内容为评论的翻译:  

当检测到问题时，这些标签会由验证管道自动应用。

| 标签                                    | 你可以做什么                                                                                                 |
|---------------------------------------|--------------------------------------------------------------------------------------------------------|
| Binary-Validation-Error               | —                                                                                                      |
| Changes-Requested                     | 做请求的修改                                                                                                 |
| Driver-Install                        | 查找开关或预安装驱动程序的方法 / 关闭 PR，因为 WinGet 不支持驱动程序                                                              |
| Error-Hash-Mismatch                   | 使用正确的哈希更新清单 (见 [Error-Hash-Mismatch](#Error-Hash-Mismatch))                                            |
| Error-Installer-Availability          | 使用新的 URL 更新清单 / 重试拉取请求                                                                                 |
| Highest-Version-Removal               | 提供删除的理由以将其推到需要关注 (Needs-Attention)                                                                     |
| Last-Version-Removal                  | 提供删除的理由以将其推到需要关注 (Needs-Attention)                                                                     |
| Manifest-AppsAndFeaturesVersion-Error | 使用正确的 ARP 条目更新清单                                                                                       |
| Manifest-Installer-Validation-Error   | 使用正确的 MSIX 信息更新清单                                                                                      |
| Manifest-Singleton-Deprecated         | 将清单更新为多清单格式 (见 [microsoft/winget-pkgs#181590](https://github.com/microsoft/winget-pkgs/issues/181590)) |
| Manifest-Validation-Error             | 更新清单以符合 JSON 架构要求                                                                                      |
| Manifest-Version-Deprecated           | 将清单更新到较新的架构版本                                                                                          |
| Manifest-Version-Error                | 将清单更新到较新的架构版本                                                                                          |
| Pull-Request-Error                    | 确保只有一个清单 / 确保清单位于正确的文件夹下                                                                               |
| URL-Validation-Error                  | 将 URL 提交给 SmartScreen 进行验证，稍后重新运行 PR 验证                                                                |
| Validation-404-Error                  | 更新安装程序网址                                                                                               |
| Validation-Defender-Error             | 将 URL 提交给 Defender 进行验证，稍后重新运行 PR 验证 (见 [Validation-Defender-Error](#Validation-Defender-Error))       |
| Validation-Domain                     | 提供域相关性的证据以将 PR 推送到需要关注 (Needs-Attention)                                                               |
| Validation-HTTP-Error                 | 将 URL 更新为使用 HTTPS (译者注：即使没有这个标签，你也应该尽量使用 HTTPS 而不是 HTTP)                                              |
| Validation-Indirect-URL               | 更新 URL 以使用直接 URL / 提供使用间接 URL 的理由                                                                      |
| Validation-Installation-Error         | 在本地测试 PR / 查看流水线日志中缺失的依赖项 / 寻求澄清或帮助 (有关日志的信息见 [验证管道日志在哪看?](#验证管道日志在哪看？))                               |
| Validation-Open-Url-Failed            | 更新 URL                                                                                                 |
| Validation-Shell-Execute              | 在本地测试 PR / 查看流水线日志中缺失的依赖项 / 寻求澄清或帮助 (有关日志的信息见 [验证管道日志在哪看?](#验证管道日志在哪看？))                               |
| Validation-Unattended-Failed          | 在本地测试 PR / 查看流水线日志中缺失的依赖项 / 寻求澄清或帮助 (有关日志的信息见 [验证管道日志在哪看?](#验证管道日志在哪看？)，关注错误截图)                        |

当一个问题无法被自动验证或需要政策决策时，这些标签由存储库维护者应用。

> 译者注：版主也可以用评论让机器人应用标签。

| 标签                                     | 你可以做什么                                                 | 注释          |
|----------------------------------------|--------------------------------------------------------|-------------|
| Interactive-Only-Installer             | 关闭这个 PR，因为安装程序不支持静默安装                                  | 手动          |
| License-Blocks-Install                 | 添加协议（Agreements）部分                                     | 手动          |
| Manifest-Content-Incomplete            | 使用足够的信息更新清单                                            | 手动          |
| Network-Blocker                        | 联系独立软件供应商（ISV）以将 Azure 虚拟机 IP 列入白名单 / 关闭 PR，因为无法采取任何措施 | 手动          |
| Portable-Archive                       | 在客户端添加支持之前关闭PR                                         | 手动          |
| Portable-Jar                           | 在客户端添加支持之前关闭PR                                         | 手动          |
| Scripted-Application                   | 在政策更改之前关闭 PR                                           | 手动          |
| Validation-Error                       | 在本地测试该PR，并确保它可以正确安装                                    | 手动          |
| Validation-Merge-Conflict              | 解决合并冲突                                                 | 手动          |
| Validation-Missing-Dependency          | 添加依赖                                                   | 手动          |
| WindowsFeatures                        | 添加对 Windows 功能的依赖                                      | 手动 / 看起来没使用 |
| Zip-Binary                             | 更新清单以包含 ArchiveBinariesDependOnPath                    | 手动          |
| 在没有仓库权限的情况下修改 validation-pipeline.yaml | 从 PR 中移除该文件                                            | 手动          |
| No-Recent-Activity                     | 回复拉取请求                                                 | 手动          |

另请参阅 [Validation Guide](https://github.com/microsoft/winget-pkgs/blob/master/doc/ValidationFailureGuide.md)。
...
### 我希望能加快审查速度
请加入 [Gitter Chat](https://gitter.im/Microsoft/winget-pkgs)，然后 _友善地_ 请求任何有空的版主批准。  
在 Chat 中有很多请求示例可以参考，往前翻翻就有。请用英文请求。  
如果您的 PR 还没有通过验证，请注意社区版主不能豁免许多验证错误。  
如果你只是想让版主重新运行验证管道，你完全可以自己关闭然后重新打开 PR 来触发运行（不要用 `@wingetbot run` / `@wingetvalidator-prod[bot] run`，这些是给版主用的）。  
他们最近审核确实挺慢的。
...
> - 建议阅读讨论 <span title="此讨论转换自议题 microsoft/winget-pkgs#333585"><a href="https://github.com/microsoft/winget-pkgs/discussions/333858">microsoft/winget-pkgs#333858</a></span> 了解现状。
...
### wingetbot / wingetvalidator-prod[bot] 是人吗？
人机。(~~你说 bot 是不是人？~~)  
...
> [!CAUTION]
> 之前使用 ADO 进行验证的方式已弃用，自 2026 年 8 月 10 日起改用 wingetvalidator-prod[bot]。  
> 这里的信息不再适用，将在近期修改。  
> 更多信息，请参阅 [microsoft/winget-pkgs#263724](https://github.com/microsoft/winget-pkgs/issues/263724)。
